...
class DOS
{
  ...
  public function file_upload($file)
  {
    ...
    if ($regex_string == '*' || !strlen($regex_string)) {
      ...
      $regex = preg_match($regex_string, basename($file));
    }
    ...
    try {
      ...
      if (is_readable($file) && !$regex) {
        ...
        $stream = fopen($file, 'r+');
        $filesystem->writeStream($this->file_path($file), $stream, [
          'visibility' => 'public'
        ]);
  ...
  public function file_delete($file)
  {
    ...
    try {
      ...
      // check if readable and regex matched
      if (file_exists($file) && !$regex) {

        // delete file in storage
        $filesystem->delete($this->file_path($file));
